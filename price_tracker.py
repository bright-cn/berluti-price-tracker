"""
Berluti Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Berluti.

Docs  : https://docs.bright.cn/scraping-automation/web-scraper-api/overview
Info  : https://bright.cn/products/insights/price-tracker/berluti
"""
...
if not API_TOKEN:
    raise EnvironmentError(
        ...
        "1. Get your token at: https://docs.bright.cn/general/account/account-settings#api-token\n"
        "2. Add it to your .env file: BRIGHTDATA_API_TOKEN=<your_token>"
    )

# Find your Web Scraper ID at: https://bright.cn/cp/scrapers
# Search for "Berluti" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://bright.cn/cp/scrapers\n"
        "2. Search for 'Berluti' and copy the Web Scraper ID.\n"
        "3. Add it to your .env file: BRIGHTDATA_DATASET_ID=<your_scraper_id>"
    )

_BASE = "https://api.bright.cn/datasets/v3"
_HEADERS = {
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Berluti product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Berluti products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Berluti category page.

    Example:
        results = discover_by_category("https://berluti.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.berluti.com/en/products/sample-product-123456",
        # Add more Berluti product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "berluti_prices.json"
    ...
    # cat_results = discover_by_category("https://berluti.com/category/sample", limit=50)
